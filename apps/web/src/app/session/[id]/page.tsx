...
type SessionState = "idle" | "intent-pending" | "streaming" | "rejected" | "complete";
...
const MOCK_INTENT_TEXT = "read prompt, generate response, pay $0.0004";
...
export default function SessionPage({ params }: SessionPageProps) {
  ...
  const [sessionState, setSessionState] = React.useState<SessionState>("intent-pending");
      ...
      <div className="max-w-4xl mx-auto px-6 py-12">
        ...
        >
          ...
          >
            ...
            {isIntentPending && (
              <p className="text-[var(--color-muted,#888)]">
                <span className="text-[#00d184]">$</span> cloudagi invoke {agentId} --prompt &quot;
                {savedPrompt}&quot;
              </p>
            )}
            {(isStreaming || isComplete) && (
              <>
                ...
                <p className="text-[var(--color-foreground,#f5f5f5)] leading-relaxed">
                  {streamedText}
                  {isStreaming && <span className="text-[#00d184] animate-pulse">_</span>}
                </p>
              </>
            )}
            {isRejected && <p className="text-yellow-400">Rejected. Session cancelled.</p>}
          </div>

          {/* Token counter — no label text to avoid duplicate /tokens|tok/ matches */}
          <div
            className="px-6 pb-3 text-xs font-mono text-[var(--color-muted,#888)]"
        ...
        >
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <p className="text-xs font-mono text-[var(--color-muted,#888)]">
              Remaining budget:{" "}
              <span className="text-[#00d184]">{budget !== "" ? `$${budget}` : "$0.00"}</span>
            </p>
          </div>
        </section>
          ...
          >
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-xs font-mono mb-4">
              ...
              <div className="sm:col-span-2">
                <dt className="text-[var(--color-muted,#888)] mb-1">Tokens in / tokens out</dt>
              ...
              <div>
                <dt className="text-[var(--color-muted,#888)] mb-1">Injection score</dt>
                <dd className="text-[var(--color-foreground,#f5f5f5)]">
        ...
        {showPostComplete && (
          <div className="flex flex-wrap items-center gap-4">
            ...
            {!ratingSubmitted ? (
              ...
              <p className="text-xs font-mono text-[#00d184]">Thanks for your rating!</p>
            )}
          </div>
        )}
      </div>
